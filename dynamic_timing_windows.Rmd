...
# So now we also do all the window measures again comparing to the same school grade (instead of same age) - grade dynamic timing windows. At the end we will compare all three (age residual, age DTW, grade DTW)

```{r}
data_g <- read.csv(file.path(workdir,"projects/W1_W2_pubertal_timing/Timing_allways_withgrade.csv", fsep=""))
```
...
# Wave 1:
data_g_w1 <- data_g[which(data_g$wave=='1'),]

for (i in 1:nrow(data_g_w1)) { 
  comp_values <- data.frame()
  
  curr_grade <- data_g_w1$grade[i]

  # modify dataframe
  temp_data_g_w1 <- data_g_w1[-i, ] # -i b/c don't want to compare to themselves
  
  for (j in 1:nrow(temp_data_g_w1)) {
    curr_comp_values <- ifelse(temp_data_g_w1$grade[j] == curr_grade, temp_data_g_w1$ldstage[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  # then go back to the original loop and subtract the mean of the current vector of values from the original individual's value  
  comp_values <- as.numeric(comp_values)
  data_g_w1$gradewindow_ldstage[i] <- data_g_w1$ldstage[i] - mean(comp_values, na.rm = TRUE)
}

# Wave 2:
data_g_w2 <- data_g[which(data_g$wave=='2'),]

for (i in 1:nrow(data_w2)) { 
  comp_values <- data.frame()
  curr_grade <- data_g_w2$grade[i]
  
  # modify dataframe
  temp_data_g_w2 <- data_g_w2[-i, ] # -i b/c don't want to compare to themselves
  
  for (j in 1:nrow(temp_data_g_w2)) {
    curr_comp_values <- ifelse(temp_data_g_w2$grade[j] == curr_grade, temp_data_g_w2$ldstage[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  # then go back to the original loop and subtract the mean of the current vector of values from the original individual's value  
  comp_values <- as.numeric(comp_values)
  data_g_w2$gradewindow_ldstage[i] <- data_g_w2$ldstage[i] - mean(comp_values, na.rm = TRUE)
}
```
...
for (i in 1:nrow(data_g_w1)) { 
  comp_values <- data.frame()
  curr_grade <- data_g_w1$grade[i]
  temp_data_g_w1 <- data_g_w1[-i, ] 
  
  for (j in 1:nrow(temp_data_g_w1)) {
    curr_comp_values <- ifelse(temp_data_g_w1$grade[j] == curr_grade, temp_data_g_w1$pdsstage[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w1$gradewindow_pdsstage[i] <- data_g_w1$pdsstage[i] - mean(comp_values, na.rm = TRUE)
}
...
for (i in 1:nrow(data_w2)) { 
  comp_values <- data.frame()
  curr_grade <- data_g_w2$grade[i]
  temp_data_g_w2 <- data_g_w2[-i, ] 
  
  for (j in 1:nrow(temp_data_g_w2)) {
    curr_comp_values <- ifelse(temp_data_g_w2$grade[j] == curr_grade, temp_data_g_w2$pdsstage[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w2$gradewindow_pdsstage[i] <- data_g_w2$pdsstage[i] - mean(comp_values, na.rm = TRUE)
}
```
...
for (i in 1:nrow(data_g_w1)) { 
  ...
  for (j in 1:nrow(temp_data_g_w1)) {
    curr_comp_values <- ifelse(temp_data_g_w1$grade[j] == curr_grade, temp_data_g_w1$ADRENcomp[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w1$gradewindow_ADRENcomp[i] <- data_g_w1$ADRENcomp[i] - mean(comp_values, na.rm = TRUE)
}
...
for (i in 1:nrow(data_w2)) { 
  ...
  for (j in 1:nrow(temp_data_g_w2)) {
    curr_comp_values <- ifelse(temp_data_g_w2$grade[j] == curr_grade, temp_data_g_w2$ADRENcomp[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w2$gradewindow_ADRENcomp[i] <- data_g_w2$ADRENcomp[i] - mean(comp_values, na.rm = TRUE)
}
```
...
for (i in 1:nrow(data_g_w1)) { 
  ...
  for (j in 1:nrow(temp_data_g_w1)) {
    curr_comp_values <- ifelse(temp_data_g_w1$grade[j] == curr_grade, temp_data_g_w1$GONADcomp[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w1$gradewindow_GONADcomp[i] <- data_g_w1$GONADcomp[i] - mean(comp_values, na.rm = TRUE)
}
...
for (i in 1:nrow(data_w2)) { 
  ...
  for (j in 1:nrow(temp_data_g_w2)) {
    curr_comp_values <- ifelse(temp_data_g_w2$grade[j] == curr_grade, temp_data_g_w2$GONADcomp[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w2$gradewindow_GONADcomp[i] <- data_g_w2$GONADcomp[i] - mean(comp_values, na.rm = TRUE)
}
```
...
for (i in 1:nrow(data_g_w1)) { 
  ...
  for (j in 1:nrow(temp_data_g_w1)) {
    curr_comp_values <- ifelse(temp_data_g_w1$grade[j] == curr_grade, temp_data_g_w1$PUBcomp[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w1$gradewindow_PUBcomp[i] <- data_g_w1$PUBcomp[i] - mean(comp_values, na.rm = TRUE)
}
...
for (i in 1:nrow(data_w2)) { 
  ...
  for (j in 1:nrow(temp_data_g_w2)) {
    curr_comp_values <- ifelse(temp_data_g_w2$grade[j] == curr_grade, temp_data_g_w2$PUBcomp[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w2$gradewindow_PUBcomp[i] <- data_g_w2$PUBcomp[i] - mean(comp_values, na.rm = TRUE)
}
```
...
for (i in 1:nrow(data_g_w1)) { 
  ...
  for (j in 1:nrow(temp_data_g_w1)) {
    curr_comp_values <- ifelse(temp_data_g_w1$grade[j] == curr_grade, temp_data_g_w1$parent_pdsstage[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w1$gradewindow_parent_pdsstage[i] <- data_g_w1$parent_pdsstage[i] - mean(comp_values, na.rm = TRUE)
}
...
for (i in 1:nrow(data_w2)) { 
  ...
  for (j in 1:nrow(temp_data_g_w2)) {
    curr_comp_values <- ifelse(temp_data_g_w2$grade[j] == curr_grade, temp_data_g_w2$parent_pdsstage[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w2$gradewindow_parent_pdsstage[i] <- data_g_w2$parent_pdsstage[i] - mean(comp_values, na.rm = TRUE)
}
```
...
for (i in 1:nrow(data_g_w1)) { 
  ...
  for (j in 1:nrow(temp_data_g_w1)) {
    curr_comp_values <- ifelse(temp_data_g_w1$grade[j] == curr_grade, temp_data_g_w1$DHEAmean[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w1$gradewindow_DHEAmean[i] <- data_g_w1$DHEAmean[i] - mean(comp_values, na.rm = TRUE)
}
...
for (i in 1:nrow(data_w2)) { 
  ...
  for (j in 1:nrow(temp_data_g_w2)) {
    curr_comp_values <- ifelse(temp_data_g_w2$grade[j] == curr_grade, temp_data_g_w2$DHEAmean[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w2$gradewindow_DHEAmean[i] <- data_g_w2$DHEAmean[i] - mean(comp_values, na.rm = TRUE)
}
``` 
...
for (i in 1:nrow(data_g_w1)) { 
  ...
  for (j in 1:nrow(temp_data_g_w1)) {
    curr_comp_values <- ifelse(temp_data_g_w1$grade[j] == curr_grade, temp_data_g_w1$TESTmean[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w1$gradewindow_TESTmean[i] <- data_g_w1$TESTmean[i] - mean(comp_values, na.rm = TRUE)
}
...
for (i in 1:nrow(data_w2)) { 
  ...
  for (j in 1:nrow(temp_data_g_w2)) {
    curr_comp_values <- ifelse(temp_data_g_w2$grade[j] == curr_grade, temp_data_g_w2$TESTmean[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w2$gradewindow_TESTmean[i] <- data_g_w2$TESTmean[i] - mean(comp_values, na.rm = TRUE)
}
```  
...
for (i in 1:nrow(data_g_w1)) { 
  ...
  for (j in 1:nrow(temp_data_g_w1)) {
    curr_comp_values <- ifelse(temp_data_g_w1$grade[j] == curr_grade, temp_data_g_w1$ESTmean[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w1$gradewindow_ESTmean[i] <- data_g_w1$ESTmean[i] - mean(comp_values, na.rm = TRUE)
}
...
for (i in 1:nrow(data_w2)) { 
  ...
  for (j in 1:nrow(temp_data_g_w2)) {
    curr_comp_values <- ifelse(temp_data_g_w2$grade[j] == curr_grade, temp_data_g_w2$ESTmean[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w2$gradewindow_ESTmean[i] <- data_g_w2$ESTmean[i] - mean(comp_values, na.rm = TRUE)
}
```      
...
for (i in 1:nrow(data_g_w1)) { 
  ...
  for (j in 1:nrow(temp_data_g_w1)) {
    curr_comp_values <- ifelse(temp_data_g_w1$grade[j] == curr_grade, temp_data_g_w1$DHEA_cor[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w1$gradewindow_DHEA_cor[i] <- data_g_w1$DHEA_cor[i] - mean(comp_values, na.rm = TRUE)
}
...
for (i in 1:nrow(data_w2)) { 
  ...
  for (j in 1:nrow(temp_data_g_w2)) {
    curr_comp_values <- ifelse(temp_data_g_w2$grade[j] == curr_grade, temp_data_g_w2$DHEA_cor[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w2$gradewindow_DHEA_cor[i] <- data_g_w2$DHEA_cor[i] - mean(comp_values, na.rm = TRUE)
}
``` 
...
for (i in 1:nrow(data_g_w1)) { 
  ...
  for (j in 1:nrow(temp_data_g_w1)) {
    curr_comp_values <- ifelse(temp_data_g_w1$grade[j] == curr_grade, temp_data_g_w1$TEST_cor[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w1$gradewindow_TEST_cor[i] <- data_g_w1$TEST_cor[i] - mean(comp_values, na.rm = TRUE)
}
...
for (i in 1:nrow(data_w2)) { 
  ...
  for (j in 1:nrow(temp_data_g_w2)) {
    curr_comp_values <- ifelse(temp_data_g_w2$grade[j] == curr_grade, temp_data_g_w2$TEST_cor[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w2$gradewindow_TEST_cor[i] <- data_g_w2$TEST_cor[i] - mean(comp_values, na.rm = TRUE)
}
```  
...
for (i in 1:nrow(data_g_w1)) { 
  ...
  for (j in 1:nrow(temp_data_g_w1)) {
    curr_comp_values <- ifelse(temp_data_g_w1$grade[j] == curr_grade, temp_data_g_w1$EST_cor[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w1$gradewindow_EST_cor[i] <- data_g_w1$EST_cor[i] - mean(comp_values, na.rm = TRUE)
}
...
for (i in 1:nrow(data_w2)) { 
  ...
  for (j in 1:nrow(temp_data_g_w2)) {
    curr_comp_values <- ifelse(temp_data_g_w2$grade[j] == curr_grade, temp_data_g_w2$EST_cor[j], NA)
    comp_values <- c(comp_values, curr_comp_values)
  }
  comp_values <- as.numeric(comp_values)
  data_g_w2$gradewindow_EST_cor[i] <- data_g_w2$EST_cor[i] - mean(comp_values, na.rm = TRUE)
}
```      
...
#Some W2 correlations are terrible, so take a look what's going on.
...
w2_ADRENcomp_sct <- ggplot(data_w2, aes(x = resid_TEST_cor, y = window_TEST_cor)) + 
  geom_point(size = 3.5) +
  scale_color_viridis(discrete=TRUE) +
  theme_bw() +
  geom_text(aes(label=paste(tagid, "(",round(age, digits=2),")")),hjust=0, vjust=0, size = 3)
