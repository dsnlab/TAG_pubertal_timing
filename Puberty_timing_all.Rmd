...
packages <- c("lme4" , "ggplot2","tidyr","dplyr", "psych","ggcorrplot","mice","BaylorEdPsych","readxl","Amelia","MissMech")
...
#impute
all_for_imp <- all_pub_int %>% 
  mutate(wave=as.numeric(wave)) %>%
  select(-int_d,-pdsstage,-ldstage,-ADRENcomp,-GONADcomp,-PUBcomp,)
imp <- amelia(all_for_imp,m=25,p2s=0,idvars="wave",ts="age",cs="tagid",polytime=1,
             noms=c("depres_d","anx_d","distress_d","fear_d"),
              ords=c("subj_timing","parent_subj_timing","adrenf2","gonadf2","PBIP_1A","PBIP_2A"),
              logs=c("CESDC_total"))
# ISSUES
#I left out the before vs after parent pds var because it was causing collinearity issues, imputation of parent pds looks ok though
#AGE IS THE TIME SERIES VARIABLE BECAUSE USING WAVE CAUSED A COLLINEARITY ERROR, BUT NOW AGE IS NOT IMPUTED. WAVE IS COLLINEAR WITH THE HORMONE VARIABLES, I DON'T UNDERSTAND HOW
#The imputation does a shitty job for CTQ_threat, the rest looks ok
