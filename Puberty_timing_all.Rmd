...
packages <- c("lme4" , "ggplot2","tidyr","dplyr", "psych","ggcorrplot","mice","BaylorEdPsych","readxl","purr")
...
#Note: we're not imputing Parent PDS and Parent subjective timing at Wave 1, because of the findings above.
#Note 2: we're also not imputing age at menarche (added below) because missings are mostly pre-menarcheal girls

#remove duplicates (TAG077 and TAG124 wave 2)
Puberty_allways <- Puberty_allways[!duplicated(Puberty_allways[c("tagid","wave")]),]
#exclude parent pds variables
Puberty_allways_cl <- Puberty_allways %>% select(-parent_pdsstage, -parent_subj_timing) %>%
  mutate(PP=as.integer(tagid), const=1) 
#check missingness pattern
md.pattern(Puberty_allways_cl)

#impute
ini <- mice(Puberty_allways_cl, maxit=0, print=F)
meth <- ini$meth
...
pred<- quickpred(Puberty_allways_cl,exclude= 'tagid') # exclude tagid
...
pred["wave",]<- 0
post <- ini$post
post[c("subj_timing","ldstage","pdsstage","ADRENcomp","GONADcomp","PUBcomp")] <- "imp[[j]][, i] <- squeeze(imp[[j]][, i], c(1, 5))" #constrain the self-report variables to their range of 1-5
imp <- mice(Puberty_allways_cl, m=20,maxit=15,meth=meth,pred=pred,post=post,print=FALSE,seed=500)

#Plotting to check the imputations
plot(imp, c("GONADcomp","ADRENcomp","PUBcomp","DHEAmean","TESTmean","ESTmean","DHEA_cor","TEST_cor","EST_cor","subj_timing"))
...
agg <- aggregate(completedData[,5:17] , by = list(completedData$.id),FUN= mean)
ids <- completedData %>% filter(.imp==1) %>% select(.id,tagid,wave) %>% mutate(Group.1=.id) 
Puberty_allways_im <- agg %>% left_join(.,ids) %>% select(tagid, everything(), -Group.1,-.id)

#check distribution imputed variables
#put in short format
Puberty_allways_short <- Puberty_allways %>% pivot_wider(id_cols= tagid, names_from = wave, names_prefix = "wave", values_from = c(age,pdsstage,ldstage,parent_pdsstage,ADRENcomp,GONADcomp,PUBcomp,subj_timing,parent_subj_timing,DHEAmean,DHEA_cor,TESTmean,TEST_cor,ESTmean,EST_cor)) 
Puberty_allways_im_short <- Puberty_allways_im %>% pivot_wider(id_cols= tagid, names_from = wave, names_prefix = "wave", values_from = c(age,pdsstage,ldstage,ADRENcomp,GONADcomp,PUBcomp,subj_timing,DHEAmean,DHEA_cor,TESTmean,TEST_cor,ESTmean,EST_cor)) 
#plot
Puberty_allways_short %>% 
  select(-tagid) %>% gather() %>%
  ggplot(aes(value)) + facet_wrap(~ key, scales = "free") + geom_histogram()
Puberty_allways_im_short %>% 
  select(-tagid) %>% gather() %>%
  ggplot(aes(value)) + facet_wrap(~ key, scales = "free") + geom_histogram()
...
Timing_allways <- Puberty_allways_short[,"tagid"]
...
for(Pub in pubcolumns1){
  modelap1 <- lm(formula = paste(Pub, "~ age_wave1"),data=Puberty_allways_short,na.action=na.exclude)
  summary(modelap1)
...
for(Pub in pubcolumns2){
  modelap2 <- lm(formula = paste(Pub, "~ age_wave2"),data=Puberty_allways_short,na.action=na.exclude)
  summary(modelap2)
  Timing_allways[[paste('resid',Pub, sep="_")]] <- resid(modelap2)
}

#create residuals for each wave separately AFTER IMPUTATION
pubcolumns1a <- list("ldstage_wave1","pdsstage_wave1","ADRENcomp_wave1","GONADcomp_wave1","PUBcomp_wave1","DHEAmean_wave1","TESTmean_wave1","ESTmean_wave1","DHEA_cor_wave1","TEST_cor_wave1","EST_cor_wave1")
for(Pub in pubcolumns1a){
  modelap1a <- lm(formula = paste(Pub, "~ age_wave1"),data=Puberty_allways_im_short,na.action=na.exclude)
  summary(modelap1a)
  Timing_allways[[paste('resid_im',Pub, sep="_")]] <- resid(modelap1a)
}

pubcolumns2a <- c("ldstage_wave2","pdsstage_wave2","ADRENcomp_wave2","GONADcomp_wave2","PUBcomp_wave2","DHEAmean_wave2","TESTmean_wave2","ESTmean_wave2","DHEA_cor_wave2","TEST_cor_wave2","EST_cor_wave2")
for(Pub in pubcolumns2a){
  modelap2a <- lm(formula = paste(Pub, "~ age_wave2"),data=Puberty_allways_im_short,na.action=na.exclude)
  summary(modelap2a)
  Timing_allways[[paste('resid_im',Pub, sep="_")]] <- resid(modelap2a)
}
...
```{r save}
write.csv(Timing_allways,paste0(cas_dir,'projects/W1_W2_pubertal_timing/Timing_allways_new.csv'),row.names=F)
