...
all_pub_int <- all_pub_int %>% merge(.,have_parPDS,by=c("tagid","wave"),all.x=T) %>%
...
all_for_imp <- all_pub_int %>% 
  ...
  select(-PUBcomp,-ADRENcomp,-int_d,-before_after_parentPDSw1)
imp <- amelia(all_for_imp,m=25,p2s=0,idvars="wave",ts="age",cs="tagid",polytime=1,
              noms=c("depres_d","anx_d","distress_d","fear_d"),
              ords=c("subj_timing","parent_subj_timing"))
#!!!! ADD BOUNDS AND LOG-TRANFORM SKEWED VARIABLES
#!!!! FIND SOLUTION FOR ADRENCOMP AND GONADCOMP CAUSING COLLEINEARITY 
#!!!! LEAVE OUT THE BEFORE AND AFTER PARENT PDS VAR?
#RECALCULATE PUBCOMP AND INT_D
