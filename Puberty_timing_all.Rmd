...
Puberty_all <- merge(PubertyComposite[,c("tagid","wave","adrenf2","gonadf2","PBIP_1A","PBIP_2A")], ChildPDS[,c("tagid","wave","subj_timing")], by=c("tagid","wave"),all=T)
...
imp <- amelia(all_for_imp,m=25,p2s=0,idvars="wave",ts="age",cs="tagid",polytime=1,
             noms=c("depres_d","anx_d","distress_d","fear_d"),
              ...
              logs=c("CESDC_total","SCARED_mean","CTQ_threat"))
#!!!! ISSUES
...
#AGE IS THE TIME SERIES VARIABLE BECAUSE USING WAVE CAUSED COLLINEARITY ERROR, BUT NOW AGE IS NOT IMPUTED. WAVE IS COLLINEAR WITH THE HORMONE VARIABLES, I DON'T UNDERSTAND HOW
#The imputation does a shitty job for SCARED_mean and CTQ_threat, the rest looks ok
...
all_im <- all_im %>%
  mutate(ldstage=(PBIP_1A+PBIP_2A)/2,
         pdsstage=(adrenf2+gonadf2)/2,
         ADRENcomp=(adrenf2+PBIP_2A)/2,
         GONADcomp=(gonadf2+PBIP_1A)/2,
         PUBcomp=mean(gonadf2,PBIP_1A,adrenf2,PBIP_2A),
