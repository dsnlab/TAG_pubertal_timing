...
#covariates
SES <- read_xlsx(paste0(cas_dir,"behavior/Demographics/ParentQ_SES/TAG_W1_SES_ParentQ_finalsamplesummarized.xlsx"),sheet=1) %>% mutate(Parent_ed=rowMeans(SES[,c("W1_parent_ed","W1_spouse_ed")],na.rm=T))
all_for_imp <- all_for_imp %>% merge(.,have_parPDS,by=c("tagid","wave"),all.x=T) %>%
                              merge(.,SES[,c("tagid","Parent_ed")],by=c("tagid"),all.x=T)
...
pred<- quickpred(all_for_imp,exclude= 'tagid') # exclude tagid
...
for(Pub in pubcolumns1a){
  modelap1a <- lm(formula = paste(Pub, "~ age_wave1"),data=all_im_short,na.action=na.exclude)
  summary(modelap1a)
...
for(Pub in pubcolumns2a){
  modelap2a <- lm(formula = paste(Pub, "~ age_wave2"),data=all_im_short,na.action=na.exclude)
  summary(modelap2a)
