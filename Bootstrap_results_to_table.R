...
mean(results_frame_imps <= obs_diff_impu,na.rm=T)
...
results_frame_long_con <-results_frame_wide_con %>% 
  pivot_longer(cols=c("all","bmictq","mhbmi","mhctq","bmi","ctq","mh","none"),names_to = "control",values_to = "effect")
...
sum(results_frame_wide_con_p$bmi < 0.05)
sum(results_frame_wide_con_p$mhctq < 0.05)
sum(results_frame_wide_con_p$mhbmi < 0.05)
sum(results_frame_wide_con_p$bmictq < 0.05)
sum(results_frame_wide_con_p$all < 0.05)
...
results_frame_long_con %>% filter(control=="none"|control=="bmi") %>% wilcox.test(effect ~ control,data=.,paired=TRUE,conf.level=0.95)
results_frame_long_con %>% filter(control=="none"|control=="all") %>% wilcox.test(effect ~ control,data=.,paired=TRUE,conf.level=0.95)

#pulling the median difference between models with and without the ctq/mh control from bootstrapped null models
results_frame_co <- data.frame("mh" = NA,"ctq" = NA,"bmi" = NA)
for(b in 1:(length(bootstraps_full)-1)){
  boot_sh <- bootstraps_full[[b]] 
  for(c in 1:ncol(results_frame_co)){
    boot_sh0 <- boot_sh %>% filter(!grepl(paste(colnames(results_frame_co[c]),"|all",sep=""),control)) %>%
    mutate(effect_0=effect) %>%
    dplyr::select(outcome,predictor,control,effect_0) %>%
    mutate(control=ifelse(control=="none",0,
                          ifelse(length(control)>4,4,
                                 ifelse(control=="bmi",1,
                                        ielse(control=="ctq",2,3)))))
    if (c==1) {
    boot_sh1 <- boot_sh %>% filter(grepl(paste(colnames(results_frame_co[c]),"|all",sep=""),control)) %>%
      mutate(effect_1=effect) %>%
      dplyr::select(outcome,predictor,control,effect_1) %>%
      mutate(control=ifelse(control=="all",4,
                            ifelse(control=="mh",0,
                                   ifelse(control=="mhbmi",1,2))))
    } else if (c==2) {
      boot_sh1 <- boot_sh %>% filter(grepl(paste(colnames(results_frame_co[c]),"|all",sep=""),control)) %>%
        mutate(effect_1=effect) %>%
        dplyr::select(outcome,predictor,control,effect_1) %>%
        mutate(control=ifelse(control=="all",4,
                              ifelse(control=="ctq",0,
                                     ifelse(control=="mhctq",3,1))))
    } else if (c==3) {
      boot_sh1 <- boot_sh %>% filter(grepl(paste(colnames(results_frame_co[c]),"|all",sep=""),control)) %>%
        mutate(effect_1=effect) %>%
        dplyr::select(outcome,predictor,control,effect_1) %>%
        mutate(control=ifelse(control=="all",4,
                              ifelse(control=="bmi",0,
                                     ifelse(control=="mhbmi",3,2))))
    }
    boot_2sh <- merge(boot_sh0,boot_sh1,by=c("outcome","predictor","control")) %>%
...
#any with mh versus any without #
round(quantile(results_frame_co["mh"], probs = c(0.025, 0.975),na.rm=T), 3)
obs_diff_mhctq <- (results_frame_wide_con$mhctq - results_frame_wide_con$ctq)
obs_diff_mhbmi <- (results_frame_wide_con$mhbmi - results_frame_wide_con$bmi)
obs_diff_mhall <- (results_frame_wide_con$all - results_frame_wide_con$bmictq)
obs_diff_mhnone <- (results_frame_wide_con$mh - results_frame_wide_con$none)
obs_diff_mh <- median(rbind(obs_diff_mhnone,obs_diff_mhctq,obs_diff_mhbmi,obs_diff_mhall))
mean(results_frame_co$mh >= obs_diff_mh,na.rm=T)
#any with ctq versus any without #
round(quantile(results_frame_co["ctq"], probs = c(0.025, 0.975),na.rm=T), 3)
obs_diff_ctqbmi <- (results_frame_wide_con$bmictq - results_frame_wide_con$bmi)
obs_diff_ctqmh <- (results_frame_wide_con$mhctq - results_frame_wide_con$mh)
obs_diff_ctqall <- (results_frame_wide_con$all - results_frame_wide_con$mhbmi)
obs_diff_ctqnone <- (results_frame_wide_con$ctq - results_frame_wide_con$none)
obs_diff_ctq <- median(rbind(obs_diff_ctqnone,obs_diff_ctqbmi,obs_diff_ctqmh,obs_diff_ctqall))
mean(results_frame_co$ctq >= obs_diff_ctq,na.rm=T)
#any with bmi versus any without#
round(quantile(results_frame_co["bmi"], probs = c(0.025, 0.975),na.rm=T), 3)
obs_diff_bmictq <- (results_frame_wide_con$bmictq - results_frame_wide_con$ctq)
obs_diff_bmimh <- (results_frame_wide_con$mhbmi - results_frame_wide_con$mh)
obs_diff_bmiall <- (results_frame_wide_con$all - results_frame_wide_con$mhctq)
obs_diff_bminone <- (results_frame_wide_con$bmi - results_frame_wide_con$none)
obs_diff_bmi <- median(rbind(obs_diff_bminone,obs_diff_bmictq,obs_diff_bmimh,obs_diff_bmiall))
mean(results_frame_co$bmi >= obs_diff_bmi,na.rm=T)
...
mean(results_frame_diffs <= obs_diff,na.rm=T)
...
# Age at menarche is not included in these statistics
#
########################################################################################
...
#set up permutation frame for results of cross-sec vs prospective
permutation_frame_w <-
  data.frame(matrix(NA, nrow = bootstraps, ncol = 56))
names(permutation_frame_w) <-
  c("permutation_number",
    "effect.tot", "sign.neg.tot", "sign.pos.tot", "sign.sig.neg.tot", "sign.sig.pos.tot",
    #ADRENcomp
    "effect.acomp","sign.neg.acomp","sign.pos.acomp","sign.sig.neg.acomp","sign.sig.pos.acomp",
...
permutation_frame_w$permutation_number <- seq.int(bootstraps)


#adjusted analyse boot function
analyse_boot_w <- function(boot_results){
  
  results_table <- data.frame(matrix(NA, nrow = 11, ncol = 3))
  names(results_table) <- c("median_effect","share_neg","share_sig_neg")
  results_table$timing_var <- c("tot","acomp","gcomp","pds","ppds","ld","subj","psubj","dhea","test","est")
...
### adjusted function for bootstrapped p values 
populate_table_bootstrap_w <- function(bootstrap, table){
...
populate_table_bounds_w <- function(boot, table, wave){
  ...
  results_frame_medians <- data.frame("parent_subj_timing" = NA,"resid_neg_ADRENcomp" = NA,
                                      "resid_neg_DHEA_cor" = NA, "resid_neg_EST_cor" = NA, "resid_neg_GONADcomp" = NA, 
                                      "resid_neg_ldstage" = NA, "resid_neg_parent_pdsstage" = NA, "resid_neg_pdsstage" = NA,
                                       "resid_neg_TEST_cor" = NA, "subj_timing" = NA, "total" = NA) #needs to be alphabetical order with total at the end
  if (wave=="wave1") {
    for(b in 1:(length(boot)-1)){
      boot_sh <- boot[[b]] %>% 
        filter(grepl(pattern="wave1", predictor)) %>% 
      ...
      for(b in 1:(length(boot)-1)){
        boot_sh <- boot[[b]] %>% 
          filter(grepl(pattern="wave2", predictor)) %>% 
          mutate(predictor= sub("_wave[1:2]|_im_wave[1:2]$", "", predictor))  
        medians <- boot_sh %>% group_by(predictor) %>% dplyr::summarise(median = median(effect))
        medians[nrow(medians)+1,2] <- median(boot_sh$effect)
        results_frame_medians <- rbind(results_frame_medians, as.numeric(t(medians)[2,]))
...
#Set up table for results of cross-sec vs prospective
results_table2a <- data.frame(predictor = NA, sig_measure = NA,
                             observed = NA, lower = NA, upper = NA, p = NA)
empty_rows <- data.frame(matrix(nrow = 32, ncol = ncol(results_table2)))
names(empty_rows) <- names(results_table2a)
results_table2a <- rbind(results_table2a, empty_rows)

results_table2a$predictor <- c("total","parent_subj_timing","resid_neg_ADRENcomp",
                              "resid_neg_DHEA_cor", "resid_neg_EST_cor", "resid_neg_GONADcomp", 
                              "resid_neg_ldstage", "resid_neg_parent_pdsstage" , "resid_neg_pdsstage",
                              "resid_neg_TEST_cor", "subj_timing")
results_table2a$sig_measure <- rep(c("median ES", "share of results", "share of sig results"),each= 11)

#Run functions
per_frame_w1 <- fill_per_w(bootstraps_full, permutation_frame_w, wave="wave1")
per_frame_w2 <- fill_per_w(bootstraps_full, permutation_frame_w, wave="wave2")
bootstrap_summary_w1 <- analyse_boot_w(per_frame_w1)
bootstrap_summary_w2 <- analyse_boot_w(per_frame_w2)

results_frame_wave1pred <- results_frame_forwide %>% filter(wave=="wave1"&!predictor=="aam_final")
results_table_wave1 <- populate_table_observed(results_frame = results_frame_wave1pred, table = results_table2a)
results_table_wave1 <- populate_table_bootstrap_w(bootstrap_summary_w1, results_table_wave1)
...
results_frame_wave2pred <- results_frame_forwide %>% filter(wave=="wave2")
results_table_wave2 <- populate_table_observed(results_frame = results_frame_wave2pred, table = results_table2a)
results_table_wave2 <- populate_table_bootstrap_w(bootstrap_summary_w2, results_table_wave2)
...
diffs_share <- per_frame_w1$sign.sig.neg.tot[1:bootstraps] - per_frame_w2$sign.sig.neg.tot[1:bootstraps]
obs_diff_share <- per_frame_w1$sign.sig.neg.tot[bootstraps+1] - per_frame_w2$sign.sig.neg.tot[bootstraps+1]
